...
class Verifier:
    ...
    def open_verify_files(self, vplan):
        ...
        try:
            ...
            logger.error('    *** CANNOT OPEN RESULTS FILE %s:\n        %s', vplan.result_path, err)
            return None

        try:
            vplan.verify_data_file = open(vplan.verify_path, encoding='utf-8', mode='r')
        except BaseException as err:
            logger.error('    **!!* %s: Cannot open verify file %s', err, vplan.verify_path)
            return None
        ...
        try:
            ...
            logger.error('    !!! Cannot create directory %s for report file %s',
        ...
        try:
            ...
            logger.error('*** Cannot open file %s: Error = %s', vplan.report_path, err)
            return None
        ...
        try:
            ...
            logger.error('*** Cannot open testdata file %s: Error = %s', vplan.testdata_path, err)
            return None
        ...
        for executor in executor_list:
            for test_type in test_list:
                ...
                if test_type not in ddt_data.testDatasets:
                    logger.warning('**** WARNING: test_type %s not in testDatasets',
                                        test_type)
                ...
                for result_version_path in version_result_directories:
                    ...
                    if os.path.isfile(new_verify_plan.result_path):
                        ...
                        # logger.debug('** No results for %s, %s, %s',
    ...
    def verify_one_plan(self, vplan):
        ...
        if not vplan.read_verify_files():
            ...
            logger.warning('Cannot get data %s or %s',
                                vplan.result_path,
                                vplan.verify_path)
            return None
        ...
        if not vplan.report.save_report():
            logger.error('!!! Could not save report for (%s, %s)',
    ...
    def setup_paths(self, executor, testfile, verify_file):
        ...
        if self.debug > 1:
            logger.debug('&&& FILE BASE = %s', base_dir)
        ...
        logger.debug('RESULT PATH = %s', self.resultPath)
        logger.debug('VERIFY PATH = %s', self.verifyPath)
        logger.debug('TESTDATA PATH = %s', self.testdata_path)
    ...
    def schema_results(self):
        ...
        if os.path.exists(schema_validation):
            ...
            try:
                ...
                logger.warning('%s. Cannot copy schema validation file from %s to %s',
...
class Tester:
    ...
    def print_result(self):
        ...
        logger.info('  Report: %s', report_data)
...
def main(args):
    ...
    if not verifier.options.summary_only:
        ...
        logger.info('Verifier starting on %d verify cases',
                             len(verifier.verify_plans))

        # Use multiprocessing on verification
        verifier.parallel_verify_data_results()

        logger.info('Verifier completed %d data reports',
                             len(verifier.verify_plans))

    verifier.create_summary_reports()
    logger.info('Verifier completed summary report')
